  ...
  (let ((leaf (cdr (assoc :info (dtree::node-content node)))))
    (when leaf
      (let* ((word (format nil "~{~A~}" path))
	     ...
	     (attribute (if (and umlaut doubles) "ATTR=umlaut-doubles"
	...
	(if multiple-entry-p
	    (let* ((wortarten (loop for x in (rest lex-entry) collect 
		   ...
		   (nemex-entry (format nil "~A ~A ~{~A:1 ~}~A" (incf *cnt*) word wortarten attribute))
		   )
	    ...
	    (let* ((info (if (listp (first lex-entry)) lex-entry (rest lex-entry)))
		   ...
		   (nemex-entry (format nil "~A ~A ~A:1 ~A" (incf *cnt*) word wortart attribute)))
	      (format *out-stream* "~&~A~%" nemex-entry))
	    )))))
...
(defun make-nemex-entries (&optional 
			     (tofile "/Users/gune00/dfki/src/Lisp/morphix/fullform/morphix-nemex.txt")
			     (expr ".*"))
  (let ((*cnt* 0)
	(*attr-cnt* 0)
    ...
    (declare (special *out-stream* *cnt* *attr-cnt*))
...
- weights
- output to file -> OK
- create initial line for file for meta information -> OK, must be moved to first line of file manually
- copy file
